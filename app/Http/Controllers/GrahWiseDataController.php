...
class GrahWiseDataController extends Controller
{
    public function index(Request $request){
        try {
            ...
            if($request->ajax()){
                ...
                $grahwisedata = GrahViseData::query();
                ...
                <a href=' . route("grahsdata.edit", ["grahsdatum" => encrypt($grah->id)]) . ' class="btn btn-sm custom-btn me-1"> <i class="bi bi-pencil" aria-hidden="true"></i></a>
            ...
            return view('admin.grahwisedata.index',compact('grahs'));
        } catch (\Throwable $th) {
            dd($th);
            return redirect()->route('grahsdata.index')->with('error','Internal server error');
        }
    }
    ...
    public function create(){
        ...
        return view('admin.grahwisedata.create',compact('grahs'));
    }
    ...
    public function store(Request $request){
        try {
            ...
            return redirect()->route('grahsdata.index')->with('message', 'Data Saved Successfully');
    ...
    public function edit($id){
        try {
            $id = decrypt($id);
            $grahs = Grah::get();
            $grahWiseData = GrahViseData::find($id);

           return view('admin.grahwisedata.edit',compact('grahs','grahWiseData'));
        } catch (\Throwable $th) {
            return redirect()->route('grahsdata.index')->with('error','Internal server error');
        }
    }
    ...
    public function update(Request $request){
        ...
        try {
            ...
            return redirect()->route('grahsdata.index')->with('message','Data Updated SuccessFully');
